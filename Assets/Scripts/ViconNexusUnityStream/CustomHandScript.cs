            ...
            //if (segmentChild.ContainsKey(BoneName) && segments.ContainsKey(BoneName))
            {
                ...
                if (!(gapFillingStrategy == GapFillingStrategy.Ignore || gapFillingStrategy == GapFillingStrategy.FillRelative) || BonePosition != Vector3.zero)
                {
                    ...
                    else
                    {
                        ...
                        if (setPosition)
                        {
                            if (fingerId == finger_1 || fingerId == finger_2 || fingerId == finger_3 || fingerId == finger_4 || fingerId == finger_5)
                            {
                                Bone.position += Bone.forward * (handProperties.BaseNormalOffset * handProperties.JointNormalOffsetRatios(fingerId, BoneName));
                                Bone.position += Bone.right * (handProperties.BaseTangentialOffset *handProperties.JointTangetialOffsetRatios(fingerId, BoneName));
                            }
